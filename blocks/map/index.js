...
module.exports = {
    ...
    attached: function () {
        ...
        var loc = new L.latLng(parseFloat(self.$data.attributes.latitude.value),
         parseFloat(self.$data.attributes.longitude.value));
        ...
        map = L.map('map_' + self.$index,
         {zoom: 11, zoomControl: false}).setView(loc, 11);


        /*Create tiles for map
          NOTE: OSM IS FOR TESTING PURPOSES ONLY - it is against their terms of
                use to use in the application
          OSM - Do no not use in app - http://{s}.tile.osm.org/{z}/{x}/{y}.png
          MapQuest OSM - Free to use in applications -
        ...
        */
        var tiles = new L.tileLayer(
            'http://otile{s}.mqcdn.com/tiles/1.0.0/map/{z}/{x}/{y}.jpeg',
            {
                ...
                'target=\'_blank\'>MapQuest</a> <img src=\'http://devel' +
                'oper.mapquest.com/content/osm/mq_logo.png\'>',
                updateWhenIdle: true,
                reuseTiles: true,
                subdomains: '1234'
            });
        ...
        marker.bindPopup('<p> <strong>' +
            self.$data.attributes.locationName.value + '</strong> <br />' +
            self.$data.attributes.address.value + '</p>');
        map.addLayer(marker);
        marker.openPopup();
